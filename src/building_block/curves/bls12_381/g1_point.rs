...
mod tests {
  ...
  fn get_g_multiples<'a>() -> Vec<G1Point> {
    ...
    let g = &G1Point::g();
    let mut gs: Vec<G1Point> = vec![g.clone()];  // gs[0] is used to match index and g's n and will not be actually used
    for p in ps {
      gs.push(p.into());
    }
    gs
  ...
  fn add_different_points() {
    ...
    let l1: G1Point = large_1.into();
    let l2: G1Point = large_2.into();
    let l3: G1Point = large_3.into();
